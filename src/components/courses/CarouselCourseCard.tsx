...
interface CarouselCourseCardProps {
  id: string;
  image: string;
  title: string;
  description: string;
...
export default function CarouselCourseCard({
  ...
  if (link) {
    return (
      <motion.a 
        href={link} 
        className="course-card-new carousel-course-card"
  ...
  return (
    <>
      <motion.div
        className="course-card-new carousel-course-card cursor-pointer"
        onClick={handleClick}
        whileHover={{ y: -4, scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        transition={{ duration: 0.2 }}
